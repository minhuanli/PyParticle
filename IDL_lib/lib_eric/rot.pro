; $Id: //depot/idl/IDL_71/idldir/lib/rot.pro#1 $
;
; Copyright (c) 1982-2009, ITT Visual Information Solutions. All
;       rights reserved. Unauthorized reproduction is prohibited.

;Rotate and magnify an image
FUNCTION ROT,Array,ANGLE,MAG,X0,y0, INTERP = interp, MISSING = missing, $
...
B = FLOAT(SIZE(Array))  ;Get dimensions
...
;
    ...
    END ELSE BEGIN
        ...
        yc = (b[2]-1)/2.
    ENDELSE
    theta = (-angle mod 360) * !dpi/180   ;angle in degrees CLOCKWISE.
...
;
    ...
    if n_elements(missing) eq 0 then $
        return,poly_2d(Array,cx,cy, i, CUBIC=cubic) $
    else return, poly_2d(Array,cx,cy, i, MISSING = missing, CUBIC=cubic)
END
