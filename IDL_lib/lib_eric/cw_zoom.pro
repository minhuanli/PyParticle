; $Id: //depot/idl/IDL_71/idldir/lib/cw_zoom.pro#1 $
;
...
PRO zoom_set_value, id, value
  ...
  state.orig_image[0:new_x_sz-1,0:new_y_sz-1] = $
     bytscl(value[0:new_x_sz-1,0:new_y_sz-1], top=!d.table_size-1)
...
PRO draw_zoom, state, newx, newy
  ...
  ;  image size.
  doerase = (rect_x GT state.x_im_sz OR rect_y GT state.y_im_sz)

  rect_x = rect_x < state.x_im_sz
  rect_y = rect_y < state.y_im_sz
  ...
  ENDIF ELSE BEGIN
    ...
    x1 = (x0 + rect_x - 1) < (state.x_im_sz-1)
    y1 = (y0 + rect_y - 1) < (state.y_im_sz-1)
    ...
    fill_x = dim_x < state.x_zm_sz
    fill_y = dim_y < state.y_zm_sz
    state.zoom_image[0:fill_x-1,0:fill_y-1] = temp_image[0:fill_x-1,0:fill_y-1]

    ; Pad with zoomed image with black if necessary.
    if (fill_x LT state.x_zm_sz) then $
      state.zoom_image[fill_x:state.x_zm_sz-1, *] = 0
    if (fill_y LT state.y_zm_sz) then $
      state.zoom_image[*, fill_y:state.y_zm_sz-1] = 0
    ...
    state.draw: $
       IF state.track GT 0 OR event.press EQ 1 THEN BEGIN
        ...
        ELSE BEGIN
            ...
            y = geo.draw_ysize - event.y
        ENDELSE
          draw_zoom, state, event.x, y
          state.oldx = event.x
          state.oldy = y
       ENDIF
...
FUNCTION cw_zoom, parent, $
  ...
  IF NOT (KEYWORD_SET(uname))  THEN uname = 'CW_ZOOM_UNAME'
