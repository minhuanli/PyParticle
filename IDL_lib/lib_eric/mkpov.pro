...
;editted by lmh @2018.12.26 tp's lab
pro colbox,pos,rgb,rad=rad
  ratio=1.3
  if (not keyword_set(rad)) then begin
    printf,1,'box { <',pos(0),strcompress(string('-R1/',ratio),/remove_all),pos(1),strcompress(string('-R1/',ratio),/remove_all),pos(2),strcompress(string('-R1/',ratio),/remove_all),'>,'
    printf,1, '<',pos(0),strcompress(string('+R1/',ratio),/remove_all),pos(1),strcompress(string('+R1/',ratio),/remove_all),pos(2),strcompress(string('+R1/',ratio),/remove_all),'>'
  endif else begin
    printf,1,'box { <',pos(0)-rad/ratio,pos(1)-rad/ratio,pos(2)-rad/ratio,'>,'
    printf,1, '<',pos(0)+rad/ratio,pos(1)+rad/ratio,pos(2)+rad/ratio,'>'
  endelse
  printf,1,'  texture { '
  if (n_elements(rgb) eq 3) then begin
printf,1,'    pigment {color rgb <',rgb(0),',',rgb(1),',',rgb(2),'>}'
...
pro mkpov,pts,filename,zmag=zmag,bw=bw,boxid = boxid, radius=radius,nobox=nobox, $
     margin=margin,camera=camera,lookat=lookat,color=color,light=light
...
radflag=n_elements(radius)-1  ; if the rad parameter is same for all particles
...
if (not keyword_set(camera)) then begin
	printf,1,'   location <',xbar+cameraoffset,ybar+cameraoffset,zbar+cameraoffset,'>'
	print,"camera at:        ",xbar+cameraoffset,ybar+cameraoffset,zbar+cameraoffset
endif else begin
	printf,1,'   location <',camera(0),camera(1),camera(2),'>'
	;printf,1,'light_source { <',camera(0),camera(1),camera(2),'> color White}'
endelse
...
printf,1,'light_source { <',xbar+cameraoffset,ybar+cameraoffset,zbar+cameraoffset,'> color White}'
endelse
printf,1,'background {color rgb <0,0,0>}'
printf,1,'#declare R1 = ',radius(0)
printf,1,'#declare R2 = ',0.007*(xres(3)-xres(2))
...
if (radflag le 0) then begin
	for i=0,nw1-1 do begin
	  ww = where(boxid eq i ,nww)
		if (not keyword_set(color)) then begin
			redsphere,tr0(*,i)
		endif else begin
		  if nww ne 0 then colbox,tr0(*,i),reform(color(*,i)) else colsphere,tr0(*,i),reform(color(*,i))
		endelse
	endfor
endif else begin
	for i=0.,nw1-1 do begin
