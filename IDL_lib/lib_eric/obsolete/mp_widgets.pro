; $Id: //depot/idl/IDL_71/idldir/lib/obsolete/mp_widgets.pro#1 $
...
;
; KEYWORD PARAMETERS:
;	NO_BLOCK:Set this keyword to zero to have XMANAGER block when this
;		application is registered.  By default NO_BLOCK is set to 1
;		to provide access to the command line if active command
;		line processing is available.  Note that setting NO_BLOCK=0
;		will cause all widget applications to block, not only this
...
pro MP_WIDGETS, request, NO_BLOCK=noblock
  ...
  IF N_ELEMENTS(noblock) EQ 0 THEN noblock=1 ELSE noblock=KEYWORD_SET(noblock)
  ...
  for i = 0, cnt-1 do begin
    tmp = STRLOWCASE(FILE_SEARCH(filepath('*.help', root_dir=lv1_dirs[i])))
  ...
  XMANAGER, 'MP_WIDGETS', base, event_handler='MPW_EVENT', NO_BLOCK=noblock
end
