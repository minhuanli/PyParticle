; $Id: //depot/idl/IDL_71/idldir/lib/obsolete/cw_bselector.pro#1 $
;
...
pro CW_BSELECTOR_SETV, id, value
...
function CW_BSELECTOR_EVENT, ev
  base = ev.handler
...
function CW_BSELECTOR, parent, names, EVENT_FUNCT = efun, $
...
;  ON_ERROR, 2						;return to caller
  ...
  WIDGET_CONTROL, top_base, MAP=map, EVENT_FUNC='CW_BSELECTOR_EVENT', $
     FUNC_GET_VALUE='CW_BSELECTOR_GETV', PRO_SET_VALUE='CW_BSELECTOR_SETV', $
     SET_UVALUE=uvalue
  ...
  menu = WIDGET_BUTTON(next_base, /MENU, value = i)
  ...
  for i = 0, n-1 do begin
    if (n_elements(font) eq 0) then begin
      ...
      ids[i] = WIDGET_BUTTON(menu, value=names[i], FONT=font, UVALUE=i)
    endelse
  endfor
