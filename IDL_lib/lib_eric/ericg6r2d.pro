...
function ericg6r2d,data,rmin=rmin,rmax=rmax,deltar=deltar,track=track
; assumes pretrack data unless /track is used
  ...
  if (nw gt 0) then begin
    ...
    for i=0L,nw-1L do begin
      ...
      dis(i)=9e9
        ...
        newp6d=real_part(p6d)
       wa=where(dis ge rvec(k) and dis lt rvec(k)+deltar,nwa)
        if (nwa gt 0) then begin
        thesito(k)=nwa
        thehistoa(k)=total(newp6d(0,wa))
        endif else begin
          thesito(k)=0.0
          thehistoa(k)=0.0
        endelse
...
theta4=checkquadrant(pos0,x0,y1,nr,rvec,rsqr,rmax,/hiy,thetax=tx,thetay=ty2)
        ...
        tempresulta=tempresulta+thehistoa/area
      ;endif
