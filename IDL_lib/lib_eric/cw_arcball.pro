; $Id: //depot/idl/IDL_71/idldir/lib/cw_arcball.pro#1 $
;
...
FUNCTION CW_ARCBALL, parent, FRAME=frame, LABEL=label, $
  ...
  IF (NOT KEYWORD_SET(uname)) THEN uname='CW_ARCBALL_UNAME'
  ...
  draw_id = widget_draw(base1, xsize = xsize, ysize=xsize, $
    ...
    UVALUE=base1, RETAIN = retain, UNAME=uname+'_DRAW')

  dummy = WIDGET_LABEL(base1, VALUE='Constraint:')
  constrain_id = WIDGET_DROPLIST(base1, $
    VALUE=['None', 'View Axes', 'Object Axes'], UVALUE=base1)

  junk = WIDGET_BASE(base1, /row)
  if keyword_set(autou) eq 0 then $
      update_id = WIDGET_BUTTON(junk, VALUE='Update', /NO_REL, $
      UVALUE=base1, UNAME=uname+'_UPDATE') $
  else update_id = 0L
...
;	4: bottom color, 5: background color.
  if n_elements(colors) ne 6 then begin
    ; Choose default colors based on decomposed setting for the device.
    device, get_decomposed=decomp
    if (decomp eq 0) then $
      ; Default to indexed colors.  These indices work well with TEK_COLOR.
      colors = [ 1,7,2,3,7,0] $
    else begin
      ; Default to decomposed colors.
      ; (white, yellow, red, green, yellow, black).
      colors = [(ISHFT(255UL,16)+ISHFT(255UL,8)+255UL), $
                (ISHFT(255UL,8)+255UL), $
                255UL, $
                (ISHFT(255UL,8)), $
                (ISHFT(255UL,8)+255UL), $
