...
;calculation dynamic heterogenity in 3d
function qt3d, xyt, a, mydts=mydts
nna=n_elements(xyt(0,*))
ttaa=max(xyt(7,*),min=ttbb)
nncc=1.*nna/(ttaa-ttbb+1)
...
for ti=0,ndt-1 do begin
  ...
  for t1=0,tmax-delt-1 do begin
    ...
    if (t2 le tmax) then begin
      ...
      for i=0,counti-1 do begin
        j=where((xj ge xi[i]-a) and (xj le xi[i]+a) and (yj ge yi[i]-a) and (yj le yi[i]+a) and (zj ge zi[i]-a) and (zj le zi[i]+a), ct)
        if ct gt 0 then begin
          xjj=xj[j]-xi[i]
          yjj=yj[j]-yi[i]
          zjj=zj[j]-zi[i]
          xy=xjj*xjj+yjj*yjj+zjj*zjj
  ...
  l=n_elements(qt)
  qchi[1,ti]=mean(qt)
  qchi[2,ti]=variance(qt)*(l-1.0)/l/nncc
