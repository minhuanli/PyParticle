; $Id: //depot/idl/IDL_71/idldir/lib/annotate.pro#1 $
;
...
if (s[1] and 7) ne 0 then $
	message, 'ANN_B_BUTTON: image width must be a multiple of 8.'
...
WIDGET_CONTROL, st.linestyle_id, SET_DROPLIST_SELECT=p[6]
...
0:	BEGIN		;Text
	...
	WIDGET_CONTROL, st.txt_font_id, SET_DROPLIST_SELECT=p[13]-3
	ENDCASE
1:	BEGIN		;Line/arrow
...
file = DIALOG_PICKFILE(FILE='annotate.ps', /WRITE, FILTER='*.ps')
IF (file EQ '') THEN RETURN ; cancel
...
SET_PLOT,'PS'
DEVICE, file=file, COLOR=st.ps_color, BITS_PER_PIXEL=8
...
case uname of
    ...
    'Mode0': ann_reset_mode, st, 0

    'Mode1': ann_reset_mode, st, 1

    'Mode2': ann_reset_mode, st, 2

    'Mode3': ann_reset_mode, st, 3

    'Mode4': ann_reset_mode, st, 4
    ...
    'Granularity': begin
        WIDGET_CONTROL, ev.id, GET_VALUE = v
        st.granularity = ann_get_num(v[0], ev.id, 1, 128)
        end

    'PSEncapsulated': st.ps_encap = ev.value

    'PSColor': st.ps_color = ev.value

    'PSLandscape': st.ps_orien = ev.value

    'PSCentimeters': st.ps_units = ev.value

    'PSWidth': begin
        WIDGET_CONTROL, ev.id, GET_VALUE = v
        st.ps_width = ann_get_num(v[0], ev.id, .1, 200)
        end
    ...
    'Save': if st.open then ann_add_object, st

    'DrawMode': begin
        redraw = 1
        st.minor_mode = ev.value
        end
    ...
    'FillMode': begin
        redraw = 1
        st.fill_mode = ev.value
        end

    'Eccentricity': begin
        redraw = 1
        st.p[18] = ev.value/10.+1
        end
    ...
    'TOPROW': begin
       ...
       'Exit' : BEGIN
        ...
        'Save As': BEGIN
    	   ...
    	   file = DIALOG_PICKFILE(FILE=st.file, /WRITE)
    		if (file eq '') then break ; cancel
    		st.file = file
    	   goto, save_file
            endelse
    	    end
        'Load': BEGIN
        	file = DIALOG_PICKFILE(FILE=st.file, /READ, /MUST_EXIST, FILTER='*.dat')
        	if (file eq '') then break ; cancel
        	st.file = file
        ...
        'TIFF': BEGIN
    	   ...
    	   file = DIALOG_PICKFILE(FILE='annotate.tif', /WRITE, FILTER='*.tif')
            if (file eq '') then break ; cancel
    	   WSET, st.backing[1]
        ...
        'PNG': BEGIN
            if (isDemo) then begin
               tmp = DIALOG_MESSAGE( /ERROR, $
                     'WRITE_PNG: Feature disabled for demo mode.')
            endif else BEGIN
    	   file = DIALOG_PICKFILE(FILE='annotate.png', /WRITE, FILTER='*.png')
    		if (file eq '') then break ; cancel
    	   WSET, st.backing[1]
    	   WRITE_PNG, file, tvrd(ORDER=0)
            endelse
    	    end
    ...
    else: MESSAGE, /INFO, 'Unknown widget: ' + uname

endcase

if (redraw && st.open) then begin
    wset, st.draw_win
...
id[0] = WIDGET_BUTTON(a, /NO_REL, value= ann_b_button(tvrd()), UNAME='Mode0')
erase

arrow, w1, w3, w9, w3, hsize = 10	;Arrows/ lines
plots, [w1, w9], wsize - [w3, w3], lines=3, /DEV
id[1] = WIDGET_BUTTON(a, /NO_REL, value= ann_b_button(tvrd()), UNAME='Mode1')
erase

polyfill, [w1,w2,w1], [w1, w9, w9], /dev	;Polygons
plots, [ 2, 4, 6, 8, 10, 12, 10, 8, 8]*wsize/16, $	;Drag/Draw
       [ 1, 3, 7, 9, 9,  11, 16, 14, 6]*wsize/20, /dev
id[2] = WIDGET_BUTTON(a, /NO_REL, value= ann_b_button(tvrd()), UNAME='Mode2')
erase
...
id[3] = WIDGET_BUTTON(a, /NO_REL, value= ann_b_button(tvrd()), UNAME='Mode3')
erase

plots, [w1,w1, w9, w9, w1], [w1, w9, w9, w1, w1], /dev   ;Squares
id[4] = WIDGET_BUTTON(a, /NO_REL, value= ann_b_button(tvrd()), UNAME='Mode4')
erase
...
st = { ANN_STATE, $
    ...
    draw_win : 0L, $
    use_xmgr: 0, $      ; 0 = uses our own event loop, 1 = uses XMANAGER
...
ann_base = WIDGET_BASE(title='Annotate', /COLUMN, $
	EVENT_PRO='annotate_event')
st.base = ann_base
...
tmp = CW_PDMENU(junk, /RETURN_NAME, UNAME='TOPROW', [ $
...
tmp = WIDGET_BUTTON(junk, value='Help', UNAME='Help', /NO_REL)
tmp = WIDGET_BUTTON(junk, value='Options', UNAME='Options', /NO_REL)

junk = WIDGET_BASE(ann_base, /ROW)
tmp = WIDGET_BUTTON(junk, VALUE='Save', UNAME='Save', /NO_REL)
st.minor_mode_id = CW_BGROUP(junk, /EXCLUSIVE, /ROW, /NO_REL, $
	LABEL_LEFT = 'Mode:', $
	['Draw', 'Edit','Select'], $
	UNAME='DrawMode', SET_VALUE=0)
...
if n_elements(tek_colors) ge 1 then begin
	...
	color_indices = (!d.table_size-1) * lindgen(ncolors) / (ncolors-1)
	device, get_decomposed=decomp
	if decomp then $
		color_indices = color_indices*256*256+color_indices*256+color_indices
endelse
...
junk = WIDGET_BASE(ann_base, /ROW, /BASE_ALIGN_CENTER)
junk1 = WIDGET_LABEL(junk, VALUE = 'Linestyle:')
st.linestyle_id = WIDGET_DROPLIST(junk, $
	VALUE=['Solid', 'Dots', 'Dashes', $
	    'Dash-Dot', 'Dash-3 Dots', 'Long Dash'], $
	UNAME='Linestyle')
...
st.txt_font_id = WIDGET_DROPLIST(junk, VALUE=fonts, $
    UNAME='Font')
st.p[13] = 3  ; default is Simplex Roman
...
junk2 = CW_BGROUP(junk1, /EXCLUSIVE, /ROW, /NO_REL, $
	['Std', 'Encapsulated'], $
	UNAME='PSEncapsulated', SET_VALUE=0)
junk2 = CW_BGROUP(junk1, /EXCLUSIVE, /ROW, /NO_REL, $
	['Mono', 'Color'], $
	UNAME='PSColor', SET_VALUE=0)
junk1 = CW_BGROUP(junk, /EXCLUSIVE, /ROW, /NO_REL, $
	['Portrait', 'Landscape'], SET_VALUE=0, $
	UNAME='PSLandscape')
junk1 = WIDGET_BASE(junk, /ROW)
...
junk2 = WIDGET_TEXT(junk1, /EDIT, /FRAME, xsize=5, ysize=1, value='5.0', $
    UNAME='PSWidth')
junk2 = CW_BGROUP(junk1, /EXCLUSIVE, /ROW, /NO_REL, $
	['In', 'Cm'], UNAME='PSCentimeters', SET_VALUE=0)

junk = WIDGET_BUTTON(top, VALUE='Dismiss', UNAME='Dismiss')
