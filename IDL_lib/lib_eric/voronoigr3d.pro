...
;------------------------------------------------------------
function subgr1,data,vvolume,rmin,rmax,deltar,lilnbar=lilnbar,enone=enone
...
;------------------------------------------------------------
function voronoigr3d,data,track=track,rmin=rmin,rmax=rmax, $
...
for t=tmin,tmax do begin
  ...
  if (nw gt 2) then begin
    ...
    tempgr=subgr1(data(0:dim-1,w),vvolume(0,w),rmin,rmax,deltar,lilnbar=lilnb,enone=enone)
