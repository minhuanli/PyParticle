;$Id: //depot/idl/IDL_71/idldir/lib/showinfo.pro#1 $
;
...
FUNCTION GetInfoText, fileName, WIDTH=width, HEIGHT=height
   ...
   IF error LT 0 THEN $
      buttonPushed = DIALOG_MESSAGE(/ERROR, $
                          [!ERROR_STATE.msg, ' Can not get information from "' + $
                           fileName + '"']) $
...
PRO ShowInfo, fileName, TITLE = title, GROUP = group, WIDTH = width, $
   ...
   Xmanager, "ShowInfo", wInfoWindow, GROUP_LEADER=group, $
   		EVENT_HANDLER="ShowInfoEventHndlr", NO_BLOCK=(NOT(FLOAT(block)))
