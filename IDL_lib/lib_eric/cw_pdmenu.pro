; $Id: //depot/idl/IDL_71/idldir/lib/cw_pdmenu.pro#1 $
;
...
pro CW_PDMENU_BUILD, parent, desc, cur, n, ev_type, full_qual_str, $
	delim, ids, mbars, HELP_KW, FONT=font, uname, context_menus
...
; Returns the ID of each button in ids.
  ...
  while (cur lt n) do begin
    ...
    if (dflags and 1) then begin
       menu=(2-mbars + context_menus)
    endif else menu = 0
   ...
   if ((mbars ne 0) and (HELP_KW ne 0) $
     ...
     new = WIDGET_BUTTON(parent, value=dname, MENU=menu, $
                         ...
                         FONT=font,  $
                         SEPARATOR=separator, $
                         UNAME=uname+'_BUTTON'+STRTRIM(cur,2))
    ...
    if (dflags and 1) then $
      ...
      new_qstr,delim,ids,mbars, 0,FONT=font,uname, context_menus
...
function CW_PDMENU, parent, desc, COLUMN=column, CONTEXT_MENU=context_menu, $
	DELIMITER=delim, FONT=font, $
  ...
  if (not KEYWORD_SET(uname)) then uname = 'CW_PDMENU_UNAME'
  ...
  context_menus = KEYWORD_SET(context_menu)
  help_kw = KEYWORD_SET(HELP_KW)
  if (mbars OR context_menus) then BEGIN
    base = parent
