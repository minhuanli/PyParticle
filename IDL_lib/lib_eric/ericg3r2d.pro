...
function ericg3r2d,data,border=border,rmin=rmin,rmax=rmax,deltar=deltar,track=track
; assumes pretrack data unless /track is used
...
y0=border(2)
y1=border(3)
x00=min(data(0,*),max=x11)
y00=min(data(1,*),max=y11)
density=npts/(1.0*(tmax-tmin+1.0))/((x11-x00)*(y11-y00))
...
for t=tmin,tmax do begin
  ...
  wb=where(data(tel,*) eq t,nw)
  data8=data(0:1,wb)
  p66=transpose(psi6(data8))
  w=where(data8(0,*) gt border(0) and data8(0,*) lt border(1) and data8(1,*) gt border(2) and data8(1,*) lt border(3),nw)
  data9=data8(*,w)
  p6=transpose(p66(0,w))
  if (nw gt 0) then begin
    ...
    for i=0L,nw-1L do begin
      pos0=data9(0:1,i);      --> the reference point
      dd=one##pos0-data9(0:1,*)
