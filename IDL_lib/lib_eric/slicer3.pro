; $Id: //depot/idl/IDL_71/idldir/lib/slicer3.pro#1 $
;
...
function Viz3D_LoadColor, CTAB=cTab, DIFFSHADE=diffShade
   ...
   if (visDepth gt 8) then displayBits = 24 else displayBits = 8
...
pro Viz3D_OrthoPlaneDraw, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   if (sMainState.sSliceState.drawMode) then begin ; Expose mode
      ...
      planeIndex = WHERE(TEMPORARY(planeDepth) gt (-32765), nPlane)
      IF (nPlane GT 0) THEN tempDepth[TEMPORARY(planeIndex)] = (-32765)
      planeDepth = 0
...
;                    PTR_FREE, (*(sMainState.hDisplayList))[i]
      ...
      'wSaveTiffBttn': begin ; Save tiff image.
        if (LMGR(/DEMO)) then begin
         ...
         tiffFiles = FILE_SEARCH(tiffFile)
         ...
         if (sMainState.sColorState.displayBits eq 24) then begin
            WRITE_TIFF, tiffFile[0], TVRD(/TRUE), ORIENTATION=4
         endif else begin
            image = TVRD()
            TVLCT, cR, cG, cB, /GET
            WRITE_TIFF, tiffFile[0], TEMPORARY(image), $
                        RED=cR, GREEN=cG, BLUE=cB, ORIENTATION=4
         endelse
         WSET, sMAinState.mainWin
      end
      ...
      'wSaveSubsetBttn': begin ; Save subset.
        if (LMGR(/DEMO)) then begin
           tmp = DIALOG_MESSAGE( /ERROR, $
                 'OPENW: Feature disabled for demo mode.')
            WIDGET_CONTROL, mainID, SET_UVALUE=sMainState, /NO_COPY
            return
         endif
         ...
         dataFiles = FILE_SEARCH(dataFile)
      ...
      'wColDiffBttn': begin ; Differential shading.
         ...
         XMANAGER, 'Viz3D_Event', diffBase, $
            EVENT_HANDLER='Viz3D_DiffShadeEvent'
      ...
      'wHelpBttn': begin ; Help
         helpFileName = filepath("idl_demo.hlp", $
                                 SUBDIR=['examples','demo','demohelp'])
         dummy=FILE_SEARCH(helpFileName, count=count)
...
pro Slicer3, hData3D, DATA_NAMES=dataNames, DETACH=detachView, $
             GROUP_LEADER=groupLead, MODAL=modal
...
COMPILE_OPT idl2
   ...
   ; Save the original decomposed setting.
   DEVICE, GET_DECOMPOSED=saveDecomp
   ...
   detachBase = KEYWORD_SET(detachView)
	runModal = KEYWORD_SET(modal)
   ; Create the widgets.

; Generic widget_base keywords for modal or nonmodal
	extra = {TITLE:'IDL 3D Data Visualizer (Slicer3)', $
		ROW:1,SPACE:1,XPAD:1,YPAD:1}
	if (N_ELEMENTS(groupLead) LE 0L) then groupLead = 0L

; Make sure we have a group leader for modal. If not, create a dummy base.
; Creating a dummy base goes against the spirit of modal bases, which should
; always have a group leader for iconization and layering. But, for backwards
; compatibility we need to not throw an error...
	IF runModal THEN BEGIN
		IF NOT WIDGET_INFO(groupLead, /VALID_ID) THEN $
			groupLead = (dummyLeader = WIDGET_BASE())
		extra =  CREATE_STRUCT(extra,'MODAL',1)
	ENDIF

	if (WIDGET_INFO(groupLead, /VALID_ID)) then $
   		extra = CREATE_STRUCT(extra,'GROUP_LEADER',groupLead)

; Create the main base, either modal or not. If not, then get menu bar
	wMainBase = runModal ? WIDGET_BASE(/MODAL, _EXTRA=extra) : $
		WIDGET_BASE(APP_MBAR=wBarBase, _EXTRA=extra)

; Left-hand column, containing all the options and buttons
	wModeBase = WIDGET_BASE(wMainBase, /COLUMN, SPACE=1, XPAD=1, YPAD=1)

; If no menu bar, then create a menu base for the buttons
	IF runModal THEN wBarBase = WIDGET_BASE(wModeBase, /ROW)
	suffix = runModal ? ' >' : ''

   WIDGET_CONTROL, wMainBase, /MANAGED
   wFileMenu = WIDGET_BUTTON(wBarBase, VALUE='File'+suffix, /MENU)
   wToolMenu = WIDGET_BUTTON(wBarBase, VALUE='Tools'+suffix, /MENU)
   wHelpMenu = WIDGET_BUTTON(wBarBase, VALUE='About'+suffix, /MENU, /HELP)
...
;                     UVALUE='wAnimateBttn')
   ...
   if (detachBase and NOT runModal) then $
...
;   wid = WIDGET_LABEL(wPerspBase, VALUE='Persp.')
   ...
   IF (visualName EQ 'TrueColor' OR visualName EQ 'DirectColor') THEN $
       DEVICE, DECOMPOSED=0
   ...
   if (runModal) then begin
      ...
      ; All done, cleanup.
		...
		IF WIDGET_INFO(dummyLeader, /VALID_ID) THEN $
			WIDGET_CONTROL, dummyLeader, /DESTROY
      ...
      ; Restore the decomposed setting
      DEVICE, DECOMPOSED=saveDecomp
