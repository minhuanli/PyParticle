; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitvissurface__define.pro#1 $
;
...
pro IDLitVisSurface::_RegisterProperties, $
    ...
    if (registerAll) then begin
        ...
        self->RegisterProperty, 'USE_DEFAULT_COLOR', /BOOLEAN, $
            ...
            DESCRIPTION='Use the surface color for the bottom'
...
pro IDLitVisSurface::GetProperty, $
                                  ...
                                  BYTESCALE_EXTENDRANGES= $
                                    byteScaleExtendRanges, $
                                  ODATA=oData, $
  ...
  if (ARG_PRESENT(bottom) || ARG_PRESENT(useDefaultColor)) then begin
...
pro IDLitVisSurface::SetProperty, $
  ...
  if (N_ELEMENTS(styleIn) && SIZE(styleIn, /TYPE) ne 7) then $
    style = LONG(styleIn)
...
PRO IDLitVisSurface::OnDataDisconnect, ParmName
  ...
  case ParmName of
    ...
    'VERTEX COLORS':begin
      ...
      self->SetPropertyAttribute, SENSITIVE=~useDefault, 'BOTTOM'
