; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitvisplot3d__define.pro#1 $
;
...
function IDLitVisPlot3d::Init, $
    ...
    shadowColor = self->_ComputeShadowColor(color)
...
pro IDLitVisPlot3d::Cleanup
    ...
    OBJ_DESTROY, self._oPolylineXYShadow
    OBJ_DESTROY, self._oPolylineYZShadow
    OBJ_DESTROY, self._oPolylineXZShadow
    OBJ_DESTROY, self._oItShadowContainer
...
;-
;----------------------------------------------------------------------------
function IDLitVisPlot3d::_ComputeShadowColor, color

    compile_opt idl2, hidden

    if (N_ELEMENTS(color) eq 3) then begin
        color_convert, color[0], color[1], color[2], h, l, s, /RGB_HLS
        ; take lightness from 0 or 1 to middle to support both black
        ; and white (dark and light) backgrounds
        l = 0.5
        ; chop saturation to dim shadow
        s = 0.5 * s
