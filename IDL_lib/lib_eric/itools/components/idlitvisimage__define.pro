; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitvisimage__define.pro#1 $
;
...
pro IDLitVisImage::_RegisterProperties, $
    ...
    if (registerAll) then begin
        ...
        self->RegisterProperty, 'ZVALUE', /FLOAT, $
            NAME='Z value', $
            DESCRIPTION='Z value for image plane'
...
pro IDLitVisImage::GetProperty, $
    ...
    if (ARG_PRESENT(byteScaleMin) or ARG_PRESENT(byteScaleMax)) then begin
...
pro IDLitVisImage::SetProperty, $
    ...
    if (N_ELEMENTS(gridUnits) ne 0) then begin
        self->_IDLitVisGrid2D::SetProperty, GRID_UNITS=gridUnits
...
function IDLitVisImage::GetDataString, xyz
    ...
    units = self._hasMapProjection ? 'meters' : self._geomUnitLabel
    value = "X:" + STRTRIM(STRING(x,FORMAT='(g0.4)'),2) + $
            "  Y: " + STRTRIM(STRING(y,FORMAT='(g0.4)'),2) + $
            " " + units
...
; the degrees symbol.
;
function IDLitVisImage::_DegToDMS, x
