; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitviscontour__define.pro#1 $
;
...
function IDLitVisContour::Init, NAME=NAME,$
   ...
   ; We assume that we are PLANAR initially.
    self._oContour = OBJ_NEW('IDLgrContour', /PLANAR, $
...
;;
;; Keywords:
;;   TWOD - set if the parameter is supposed to be a 2D array.  This
;;          allows for handling the case of one colour which seems to
;;          be a 3 element vector instead of a 3x1 array.
;;
PRO IDLitVisContour::_FillParameter, parameterName, parameter, TWOD=twod
  compile_opt idl2, hidden
    ...
    if keyword_set(n_levels) then begin
        ...
        IF (n_levels GT n_elements(c_valueTemp)) THEN BEGIN
          self->_FillParameter,'C_COLOR',cColorOld, $
            TWOD=~self._palColor && N_ELEMENTS(cColorOld) eq 3
...
;        endif
    ...
    IF n_elements(color) EQ 3 THEN BEGIN
      ...
      self->_FillParameter,'C_COLOR',color,/TWOD
    ENDIF
