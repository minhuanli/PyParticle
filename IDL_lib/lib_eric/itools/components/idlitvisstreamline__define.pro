...
pro IDLitVisStreamline::_UpdateStreamlines, sMap, minX, minY, maxX, maxY
    ...
    while (idx lt npoly) do begin
        ...
        if (self._autoColor gt 0) then begin
            ...
            if (self._autoColor eq 1) then begin
                delta[polylines[idx+1]] = TOTAL(diff^2, 1)
            endif else begin
                u = REFORM(diff[0,*])
                v = REFORM(diff[1,*])
    ...
    headSizeX = (15./xDim)*self._headSize*xrange/xDim
    headSizeY = (15./yDim)*self._headSize*yrange/yDim

    self._oSymbol->SetProperty, SIZE=[headSizeX, headSizeY, 1]
    labelObject = (nlines gt 0) ? $
        REPLICATE(self._oSymbol, nlines) : self._oSymbol
    labelPolylines = (nlines gt 0) ? LINDGEN(nlines) : 0
