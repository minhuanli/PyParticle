; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitmaniprangepan__define.pro#1 $
;
...
pro IDLitManipRangePan::_Pan, Direction, AXIS=inAxis
    ...
    nDS = self.nSelectionList
    for i=0,nDS-1 do begin
        oDataSpace = (*self.pSelectionList)[i]
        if (OBJ_VALID(oDataSpace) eq 0) then continue
