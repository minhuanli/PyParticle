; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitmanipvisscale__define.pro#1 $
;
...
;   None.
;
function IDLitManipVisScale::Init, NAME=inName, $
    _REF_EXTRA=_extra
    ...
    self.oVis = OBJ_NEW('IDLitManipVisScale2D', COLOR=[0,150,0])
    self->Add, self.oVis
...
pro IDLitManipVisScale::Set3D, is3D
    ...
    if is3D then begin
        if (OBJ_ISA(self.oVis, 'IDLitManipVisScale2D')) then $
            oVis = OBJ_NEW('IDLitManipVisScale3D')
    endif else begin
        if (OBJ_ISA(self.oVis, 'IDLitManipVisScale3D')) then $
            oVis = OBJ_NEW('IDLitManipVisScale2D')
    endelse
