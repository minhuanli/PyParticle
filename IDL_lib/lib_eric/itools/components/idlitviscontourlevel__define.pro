; $Id: //depot/idl/IDL_71/idldir/lib/itools/components/idlitviscontourlevel__define.pro#1 $
;
...
pro IDLitVisContourLevel::_RegisterProperties
   ...
   compile_opt idl2, hidden
    ...
    self->RegisterProperty, 'VALUE', /FLOAT, $
        ...
        /HIDE  ; need to disable for styles, until we have data
    ...
    self->RegisterProperty, 'LABEL_INTERVAL', /FLOAT, $
        ...
        VALID_RANGE=[0.05d,1d,0.05d]

    self->RegisterProperty, 'LABEL_NOGAPS', /BOOLEAN, $
        NAME='No label gaps', $
        DESCRIPTION='Do not have gaps for labels'
    ...
    self->RegisterProperty, 'LABEL_SYMBOL', /SYMBOL, $
        NAME='Symbol label', $
        DESCRIPTION='Contour symbol label'
...
pro IDLitVisContourLevel::SetProperty, $
    ...
    if (N_ELEMENTS(myLabelType)) then BEGIN
        ...
        ; Turn on/off value/text properties.
        self->SetPropertyAttribute, $
            ['FONT_INDEX', 'FONT_STYLE', 'FONT_SIZE'], $
            SENSITIVE=(self._labelType eq 1) || (self._labelType eq 2)
