; $Id: //depot/idl/IDL_71/idldir/lib/itools/irotate.pro#1 $
...
ON_ERROR, 2
  ...
  ;; degrees is required
  if ((N_ELEMENTS(degreesIn) eq 0) && ~KEYWORD_SET(resetIn)) then begin
    catch, /cancel
  ...
  if (~KEYWORD_SET(resetIn) && (degrees eq 0.0d)) then return
  ...
  for i=0,N_ELEMENTS(fullID)-1 do begin
    ...
    if (pos[0] eq -1) then begin
      ...
      if (~allow) then begin
        ...
        ;; Act on the dataspace root
        oObj = oDS->GetDataSpaceRoot()
      endif
    endif
    ...
    if (KEYWORD_SET(resetIn)) then begin
      scale = SQRT(TOTAL(currentTransform^2,1))
      ...
      if (oObj->is3D()) then begin
        oObj->_IDLitVisualization::Rotate, [1, 0, 0], -90, $
          CENTER_OF_ROTATION=cr, _EXTRA=_extra
