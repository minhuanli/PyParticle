; $Id: //depot/idl/IDL_71/idldir/lib/itools/framework/idlitgrlayer__define.pro#1 $
;
...
pro IDLitgrLayer::_RegisterProperties, $
    ...
    if (registerAll) then begin
        ; _PROJECTION is a "hidden" property used to override the grView
        ; keyword.  We need this because PROJECTION wants values of 1 or 2,
        ; while our ENUMLIST returns values starting at 0.
        self->RegisterProperty, '_PROJECTION', $
            NAME='Projection', $
...
pro IDLitgrLayer::GetProperty, $
    ...
    if (ARG_PRESENT(_projection) ne 0) then begin
        self->IDLgrView::GetProperty, PROJECTION=projection
        _projection = (projection - 1) > 0
    endif
...
pro IDLitgrLayer::SetProperty, $
    ...
    if (N_ELEMENTS(_projection) ne 0) then begin
        newProj = (_projection + 1) < 2
...
;   None
;
pro IDLitgrLayer::_UpdateMargins
