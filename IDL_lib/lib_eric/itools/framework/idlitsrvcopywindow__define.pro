; $Id: //depot/idl/IDL_71/idldir/lib/itools/framework/idlitsrvcopywindow__define.pro#1 $
;
...
function IDLitsrvCopyWindow::DoWindowCopy, oWindow, oSource, $
   ...
   if(isScene)then begin
    ...
    if (OBJ_ISA(self._oDest, 'IDLgrBuffer') || $
      ...
      OBJ_ISA(self._oDest, 'IDLgrWindow')) then begin
      if (isScene) then begin
...
;;   oTool
;;
function IDLitsrvCopyWindow::DoAction, oTool
    ...
    if (~OBJ_VALID(oWindow)) then $
        return, OBJ_NEW()
    self->DoCopyWindow, oWindow, oWindow->GetScene()
