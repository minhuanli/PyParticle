; $Id: //depot/idl/IDL_71/idldir/lib/itools/framework/_idlitmapprojection__define.pro#1 $
;
...
@map_proj_init_commonblock
    ...
    keep = WHERE(c_ProjNumber gt 100)
...
pro _IDLitMapProjection::_UpdateLimit
    ...
    'Stereographic': begin
        if (clat eq 0) then $
            newlimit = [-90, -180 > (clon-90), 90, (clon+90)<180]
        break
        end
...
@map_proj_init_commonblock
    ...
    'Polar Stereographic': begin
        ...
        allowed = ['CENTER_LONGITUDE', 'HEMISPHERE']
        break
        end
    ...
    'Miller Cylindrical': break  ; do nothing
...
@map_proj_init_commonblock
    ...
    if (N_ELEMENTS(strEllipsoid) eq 1) then begin
        ...
        if (match ge 0) then begin
            ...
            self->ErrorMessage, SEVERITY=1, $
                IDLitLangCatQuery('Warning:MapProjection:UnknownEllipsoid') + $
                strEllipsoid, $
                TITLE=IDLitLangCatQuery('Menu:Operations:MapProjection')
        endelse
    endif
