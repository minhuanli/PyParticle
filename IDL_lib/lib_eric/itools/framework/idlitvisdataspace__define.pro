; $Id: //depot/idl/IDL_71/idldir/lib/itools/framework/idlitvisdataspace__define.pro#1 $
;
...
pro IDLitVisDataSpace::_RegisterProperties, $
    ...
    if (registerAll) then begin
        self->RegisterProperty, 'X_MINIMUM', /FLOAT, $
            NAME='X minimum', $
            DESCRIPTION='Minimum x value', /HIDE

        self->RegisterProperty, 'X_MAXIMUM', /FLOAT, $
            NAME='X maximum', $
            DESCRIPTION='Maximum x value', /HIDE

        self->RegisterProperty, 'Y_MINIMUM', /FLOAT, $
            NAME='Y minimum', $
            DESCRIPTION='Minimum y value', /HIDE

        self->RegisterProperty, 'Y_MAXIMUM', /FLOAT, $
            NAME='Y maximum', $
            DESCRIPTION='Maximum y value', /HIDE
        ...
        self->RegisterProperty, 'X_AUTO_UPDATE', /BOOLEAN, $
            ...
            Description='Update the X range automatically'

        self->RegisterProperty, 'Y_AUTO_UPDATE', /BOOLEAN, $
            Name='Automatic Y range', $
            Description='Update the Y range automatically'
...
pro IDLitVisDataSpace::SetProperty, $
    ...
    if (N_ELEMENTS(xRangeCmdLine) ne 0) then $
        self->SetProperty, X_MINIMUM=xRangeCmdLine[0], X_MAXIMUM=xRangeCmdLine[1]
    if (N_ELEMENTS(yRangeCmdLine) ne 0) then $
        self->SetProperty, Y_MINIMUM=yRangeCmdLine[0], Y_MAXIMUM=yRangeCmdLine[1]
    if (N_ELEMENTS(zRangeCmdLine) ne 0) then $
        self->SetProperty, Z_MINIMUM=zRangeCmdLine[0], Z_MAXIMUM=zRangeCmdLine[1]
...
pro IDLitVisDataSpace::OnDataRangeChange, oSubject, XRange, YRange, ZRange, $
    ...
    if (~initializing) then begin
        self->GetProperty, _PARENT=my__parent
        if (OBJ_ISA(my__parent, 'IDLitVisNormDataSpace')) then $
            my__parent->GetProperty, INITIALIZING=initializing
    endif
    ...
    ; Make sure we have registered properties.
    if (self->QueryProperty('X_MINIMUM')) then begin
        ...
        self->SetPropertyAttribute, 'XLOG', $
            SENSITIVE=self._xLog || (newXrange[0] gt 0 && newXrange[1] gt 0)
        self->SetPropertyAttribute, 'YLOG', $
            SENSITIVE=self._yLog || (newYrange[0] gt 0 && newYrange[1] gt 0)
        self->SetPropertyAttribute, 'ZLOG', $
            SENSITIVE=self._zLog || (newZrange[0] gt 0 && newZrange[1] gt 0)
        self->SetPropertyAttribute, ['X_MINIMUM', 'X_MAXIMUM', $
            'Y_MINIMUM', 'Y_MAXIMUM'], HIDE=0
    endif
...
PRO IDLitVisDataSpace::Add, oVis, $
    ...
    if (validPositions[1] gt 0) then begin
...
function IDLitVisDataSpace::GetXYZRange, xRange, yRange, zRange, $
    ...
    if (~KEYWORD_SET(includeAxes) && (self->count() gt limit)) then begin
        ...
        return, self->_IDLitVisualization::GetXYZRange( $
       ...
       (self.axesRequest ne oldAxesRequest)) then begin
        ...
        if (self.axesRequest) then begin
            ...
            if (oldStyle eq 0) then begin
                ...
                if (style lt 0) then $
                    style = 1
                self.oAxes->SetProperty, STYLE=style
            ...
            if (nVis ne 0) then begin
                ...
                if (oldStyle ne 0) then begin
                    self.oAxes->SetProperty, STYLE=0
                    self->DoOnNotify, self.oAxes->GetFullIdentifier(), $
                        "SETPROPERTY", "STYLE"
                endif
            endif
...
pro IDLitVisDataSpace::OnAxesStyleRequestChange, oSubject, styleRequest
    ...
    if (self.axesRequest ne 0) then begin
        ...
        if (newStyle ne oldStyle) then begin
            self.oAxes->SetProperty, STYLE=newStyle
            self->DoOnNotify, self.oAxes->GetFullIdentifier(), $
                "SETPROPERTY", "STYLE"
        endif
    endif
...
function IDLitVisDataSpace::GetDataString, xyz
    ...
    if (self->Is3D()) then begin
        str = STRTRIM(STRING(xyz, FORMAT='(G11.4)'),2)
        return, STRING(str, FORMAT='("[",A,", ",A,", ",A,"]")')
    endif

    xy = STRTRIM(STRING(xyz[0:1], FORMAT='(G11.4)'),2)
    return, STRING(xy, FORMAT='("[",A,", ",A,"]")')
