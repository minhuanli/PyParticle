; $Id: //depot/idl/IDL_71/idldir/lib/itools/ui_widgets/cw_itwindow.pro#1 $
...
pro cw_itwindow_visualize_variable, varname
  ...
  if (ndim eq 1 && dims[0] gt 1) then begin
    ...
    success = IDLNotify('IDLExecute', $
      'iPlot, ' + varname + ', /OVERPLOT')

    ; If not success, must be command-line IDL. Just do the command.
  ...
  if (ndim eq 2 || (ndim eq 3 && mind le 4)) then begin
    ...
    success = IDLNotify('IDLExecute', $
      command + ', ' + varname + ', /OVERPLOT' + geotiffVar + rgbTableVar)
  ...
  for i=0,N_ELEMENTS(dropData)-1 do begin
    ...
    if (j eq nvars) then begin
      ...
      if (FILE_TEST(dropData[i])) then begin
        ...
        success = IDLNotify('IDLExecute', dropData[i] + ', /OVERPLOT')
        ; If not success, must be command-line IDL. Just do the command.
