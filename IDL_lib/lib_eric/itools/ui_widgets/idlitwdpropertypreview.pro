; $Id: //depot/idl/IDL_71/idldir/lib/itools/ui_widgets/idlitwdpropertypreview.pro#1 $
...
pro IDLitwdPropertyPreview_event, event
...
;
function IDLitwdPropertyPreview, oUI, $
    CANCEL=cancel, $
    NO_COMMIT=noCommit, $
    ...
    wRow = WIDGET_BASE(wTLB, /ROW, XPAD=0)
    ...
    wPreview = CW_ITPROPERTYPREVIEW(wRow, oUI, VALUE=oComponent)

    ; Create status bar
    wStatus = WIDGET_LABEL(wTLB, /SUNKEN_FRAME,$
        XSIZE=scrXsize - 4, $
        VALUE=' ', $
        /ALIGN_LEFT)
    ...
    wOK = Widget_Button(wButtons, VALUE=' '+IDLitLangCatQuery('UI:OK')+'  ', uname='OK')
    ...
    XMANAGER, "IDLitwdPropertyPreview", wTLB,  NO_BLOCK=0
