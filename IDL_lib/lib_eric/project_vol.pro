; $Id: //depot/idl/IDL_71/idldir/lib/project_vol.pro#1 $
;
...
FUNCTION Project_Vol, vol, x_sampleIn, y_sampleIn, z_sampleIn, $
         Depth_q=depth_qIn, Opaque=opaqueIn, Trans=transIn, Cubic=cubic, $
         Xsize=xsizeIn, Ysize=ysizeIn, Avg_Intensity=avg_intensity, $
         Z_Buffer=z_buffer
...
x_sample = Long(x_sampleIn[0])
y_sample = Long(y_sampleIn[0])
xy_sample = x_sample * y_sample
z_sample = Long(z_sampleIn[0])
zf_sample = Float(z_sample)
zf_sample_m1 = zf_sample - 1.0
z_sample_m1 = z_sample - 1L
z_max = Float(z_sample - 1L)

xsize = (n_elements(xsizeIn) > 0) ? xsizeIn : $
  ((!D.Window GE 0L) ? !D.X_Size : x_sample)

ysize = (n_elements(ysizeIn) > 0) ? ysizeIn : $
  ((!D.Window GE 0L) ? !D.Y_Size : y_sample)

depth_q = (N_ELEMENTS(depth_qIn) GT 0) ? 0.0 > Float(depth_qIn[0]) < 1.0 : 1.0
depth_q = 1.0 - depth_q

block_out = 0B
IF (N_Elements(opaqueIn) GT 0L) THEN BEGIN
   IF (N_Elements(opaqueIn) EQ N_Elements(vol)) THEN BEGIN
      opaque = Reform(opaqueIn, size_vol[1], size_vol[2], size_vol[3])
      block_out = 1B
   ENDIF ELSE BEGIN
...
IF (N_Elements(transIn) GT 0) THEN BEGIN
   IF (N_Elements(transIn) NE 16) THEN BEGIN
      Print, 'Incorrect number of elements in Trans.'
      RETURN, Bytarr(xsize, ysize)
   ENDIF ELSE BEGIN
      trans = Float(Reform(transIn, 4, 4))
   ENDELSE
ENDIF ELSE BEGIN
   trans = !P.T
ENDELSE
