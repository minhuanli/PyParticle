; $Id: //depot/idl/IDL_71/idldir/lib/mesh_obj.pro#1 $
;
...
CASE obj_type OF
   ...
   3: BEGIN ; Cylindrical surface.
      ...
      IF (Keyword_Set(closed)) THEN BEGIN
         IF (max_ang - min_ang EQ 2.0 * !PI) THEN BEGIN
             ; Mesh closes on itself - no gap.
             ; Ignore last (repeated) column of vertices and make the
             ; last column of polygons connect to the first column of vertices.
             p_ind = (Lindgen(dim_y-1) * (dim_x-1L) + (dim_x-2L)) * 5L
             polygon_list[p_ind + 2L] = polygon_list[p_ind + 2L] - (dim_x-1)
             polygon_list[p_ind + 3L] = polygon_list[p_ind + 3L] - (dim_x-1)
         ENDIF $
         ELSE BEGIN
            ; There is a gap.  Generate polygons to close it.
            ; Use the first and last columns of vertices to
            ; form the new polygons.
            indx_num = dim_y - 1L
            close_list = Lonarr(indx_num * 5L, /Nozero)
            p_ind = Lindgen(indx_num) * 5L
            close_list[p_ind] = 4L
            close_list[p_ind + 1L] = (Lindgen(indx_num) + 1L) * dim_x - 1L
            close_list[p_ind + 2L] = Lindgen(indx_num) * dim_x
            close_list[p_ind + 3L] = close_list[p_ind + 2L] + dim_x
            close_list[p_ind + 4L] = close_list[p_ind + 1L] + dim_x
            polygon_list = [polygon_list, close_list]
         ENDELSE
      ENDIF
      p_ind = 0
   ...
   4: BEGIN ; Spherical surface.
      ...
      IF (Keyword_Set(closed)) THEN BEGIN
         IF (max_lon - min_lon EQ 2.0 * !PI) THEN BEGIN
             ; Mesh closes on itself - no gap.
             ; Ignore last (repeated) column of vertices and make the
             ; last column of polygons connect to the first column of vertices.
             p_ind = (Lindgen(dim_y-1) * (dim_x-1L) + (dim_x-2L)) * 5L
   ...
   5: BEGIN ; Surface of extrusion.
      ...
      polygon_list[p_ind + 4L] = polygon_list[p_ind + 3L] - 1L
      IF (Keyword_Set(closed)) THEN BEGIN
         result = WHERE(array1[*, 0] - array1[*, max_ind], count)
         IF (count EQ 0) THEN BEGIN
             ; Mesh closes on itself - no gap.
             ; Ignore last (repeated) column of vertices and make the
             ; last column of polygons connect to the first column of vertices.
             p_ind = (Lindgen(steps) * (steps-1) + (max_ind-1)) * 5L
             polygon_list[p_ind + 2L] = polygon_list[p_ind + 2L] - (max_ind)
             polygon_list[p_ind + 3L] = polygon_list[p_ind + 3L] - (max_ind)
         ENDIF $
         ELSE BEGIN
            ; There is a gap.  Generate polygons to close it.
            ; Use the first and last columns of vertices to
            ; form the new polygons.
            close_list = Lonarr(steps * 5L, /Nozero)
            p_ind = Lindgen(steps) * 5L
            close_list[p_ind] = 4L
            close_list[p_ind + 1L] = (Lindgen(steps) + 1L) * sz_array[2] - 1
            close_list[p_ind + 2L] = Lindgen(steps) * sz_array[2]
            close_list[p_ind + 3L] = close_list[p_ind + 2L] + sz_array[2]
            close_list[p_ind + 4L] = close_list[p_ind + 1L] + sz_array[2]
            polygon_list = [polygon_list, close_list]
         ENDELSE
      ENDIF
      p_ind = 0
   ...
   6: BEGIN ; Surface of revolution.
      ...
      IF (Keyword_Set(closed)) THEN BEGIN
         IF (max_ang - min_ang EQ 2.0 * !PI) THEN BEGIN
             ...
             ; Ignore last (repeated) column of vertices and make the
             ; last column of polygons connect to the first column of vertices.
             p_ind = (Lindgen(max_ind) + max_ind * (steps-1)) * 5L
             polygon_list[p_ind + 2L] = polygon_list[p_ind + 2L] - steps * sz_array[2]
             polygon_list[p_ind + 3L] = polygon_list[p_ind + 3L] - steps * sz_array[2]
         ENDIF $
         ELSE BEGIN
            ; There is a gap.  Generate polygons to close it.
            ; Use the first and last columns of vertices to
            ...
            p_ind = Lindgen(indx_num) * 5L
            close_list[p_ind] = 4L
            close_list[p_ind + 1L] = Lindgen(indx_num)
            close_list[p_ind + 2L] = close_list[p_ind + 1L] + 1L
            close_list[p_ind + 3L] = close_list[p_ind + 2L] + steps * sz_array[2]
            close_list[p_ind + 4L] = close_list[p_ind + 1L] + steps * sz_array[2]
            polygon_list = [polygon_list, close_list]
         ENDELSE
      ENDIF
      p_ind = 0
