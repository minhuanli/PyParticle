...
;0)r^2 1)x 2)y 3)z
function selectnearest_vec,odata, cp=cp,rmax=rmax, nmax=nmax
t=round(cp(17))
...
if (j eq 0)  then begin 
   ...
   if (j le nmax) then begin
      ...
      n=n_elements(nb1(0,*))
      result=fltarr(3*nmax)
        ;flat the nblist
        for i=0,n-1 do begin
          result(3*i:(3*i+2))= nb1(1:3,i)
        endfor
...
;-----------------------------------------------------------------
function neighvec_group,odata,cp=cp,rmax=rmax,nmax=nmax

   ncp=n_elements(cp(0,*))
   result=fltarr(3*nmax,ncp)
   for i = 0.,ncp-1 do begin 
      result(*,i)= selectnearest_vec(odata, cp=cp(*,i),rmax=rmax, nmax=nmax)
   endfor
