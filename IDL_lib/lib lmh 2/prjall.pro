...
;r is the projection sphere's radius   data is the database for the search of cp1's neareast neighbor 
;---------------------------------------------------------------------------------------------------
;project all the particles' neighbors to one standard sphere
function prjall, cpdata=cpdata, data=data, rm=rm, nm=nm
s=n_elements(cpdata(0,*))
sr=5
for j=0.,(s-1) do begin
  temp=prj1(data1=data,cp1=cpdata(*,j),rm1=rm,nm1=nm)
  if (j eq 0) then begin
    pjall=temp
  endif else begin
    pjall=[[pjall],[temp]]
