...
;--------------------------------------------------------------------
;0-x 1-y 2-area 3-t
function vr2dareat,trbb 

ncol=n_elements(trbb[*,0])
tmin=long(min(trbb[ncol-1L,*]))
tmax=long(max(trbb[ncol-1L,*]))
result=[-1,-1,-1,-1]

for t=tmin,tmax do begin
  print,t
  w=where(trbb[ncol-1L,*] eq t)
  trb=trbb(*,w) 
  n1=n_elements(trb(0,*))
...
nn=n_elements(result(0,*))
result=result(*,1:(nn-1))  ; dismiss the -1 row
